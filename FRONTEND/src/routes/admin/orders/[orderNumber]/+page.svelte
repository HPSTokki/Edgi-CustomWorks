...
async function fetchOrder() {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3000/api/admin/orders/${orderNumber}`);
  ...
  async function fetchOptions() {
    try {
      const response = await fetch('http://localhost:3000/api/admin/orders/filters/options');
      const result = await response.json();
      
      if (result.success) {
        statusOptions = result.data.status.filter((opt: string) => opt !== 'all');
        paymentStatusOptions = result.data.paymentStatus.filter((opt: string) => opt !== 'all');
      }
    } catch (err) {
  ...
  async function updateStatus() {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/admin/orders/${order.orderNumber}/status`, {
        method: 'PATCH',
  ...
  async function updatePaymentStatus() {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/admin/orders/${order.orderNumber}/payment-status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function deleteOrder() {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/admin/orders/${order.orderNumber}`, {
        method: 'DELETE'
      });
              ...
              <div>
                ...
                {:else if order}
                  ...
                  <div class="mt-1 flex items-center space-x-4">
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {getPaymentStatusColor(order.paymentStatus)}">
                      {order.paymentStatus}
                    </span>
                  ...
                  {#each order.items as item}
                    <li class="px-4 py-4 sm:px-6">
                      <div class="flex items-center">
                        ...
                        {:else}
                          <div class="shrink-0 h-16 w-16 bg-gray-200 rounded-md flex items-center justify-center">
